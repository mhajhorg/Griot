...
agentRouter.post('/agent', async (req, res) => {
  try {
    ...
    if (isConversational(query) && !attachment) {
      ...
      return res.json({ summary: reply.trim(), citations: [], total_paid: 0 });
    }
    ...
    async function finalize(summary, modelCitations) {
      ...
      for (const c of modelCitations || []) {
        ...
        console.log(`[agent] finalize citation url="${c.url}" matchedRegistryEntry=`, entry ? { id: entry.id, title: entry.title, mode: entry.mode, price: entry.price } : null);

        if (!entry) {
          // Not a registered piece of content — cite as-is, no payment involved.
          citations.push({ title: c.title, url: c.url, amount_paid: 0, tx_hash: null });
          continue;
        }

        // Registered content cited in the final answer ALWAYS gets paid,
        // regardless of whether the model called a pay step — this is the
        // server-side safety net, not something left to model compliance.
      ...
      return res.json({ summary, citations, total_paid: totalPaid });
    }
    ...
    for (let turn = 0; turn < MAX_TURNS; turn++) {
      ...
      if (action.action === 'final') {
        if (!hasSearchedRegistry) {
          // Model tried to skip the mandatory registry check — force it back.
      ...
      if (action.action === 'search_registry') {
        ...
        const result = matches.map(m => {
          // Register it immediately — payment enforcement and citation
          // matching no longer depend on fetch_content ever succeeding for
          // registered content, since we already have the real text here.
      ...
      if (action.action === 'check_registry') {
        // Kept for backward compatibility in case the model uses this instead
        // of search_registry for a URL it already has in hand.
        let normalizedUrl;
        try {
      ...
      if (action.action === 'fetch_content') {
        try {
          ...
          // Record whether this URL is registered, so finalize() can charge for it
          // later even if the model never explicitly checked/paid. Normalized so
          // trivial URL variations still match the registered canonical_url (which
          // was itself stored normalized at registration).
